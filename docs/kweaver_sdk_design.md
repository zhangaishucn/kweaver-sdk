...
> **v0.6 变更**: 原四层架构（Skill → CLI → SDK → HTTP）简化为三层。Skill 层已删除，CLI 成为唯一的编排层。

```
┌───────────────────────────────────────────────┐
│  CLI 层（最上层）                              │
│  kweaver 命令行，面向 AI 智能体和终端用户       │
│  多步编排、错误处理、JSON 输出                  │
├───────────────────────────────────────────────┤
│  SDK 层（面向开发者）                           │
...
**依赖方向：** CLI 只向下引用 SDK；SDK 只向下引用 HTTP。
...
- CLI 看到的：终端用户和 AI Agent 的操作命令（`kweaver ds connect`、`kweaver kn create`），负责多步编排和 JSON 输出。CLI 是唯一的编排层，多步操作（连接→注册→发现表）在 CLI 命令内完成。
...
| | CLI | SDK |
|---|---|---|
| **用户** | AI Agent / 终端用户 | 开发者（代码调用） |
| **可引用** | SDK（不引用其他） | HTTP（不引用 CLI） |
| **认证** | ConfigAuth → ~/.kweaver/ | AuthProvider 注入 |
| **输出** | 结构化 JSON | 类型化 Python 对象 |
| **错误** | `handle_errors` → 打印并退出 | 抛出 ADPError |
