...
kweaver bkn object-type update <kn_id> <ot_id> [--name <n>] [--display-key <dk>] [--add-property '<json>' ...] [--remove-property <name> ...] [--tags '["标签"]'] [--comment <s>] [--icon <i>] [--color <c>] [--branch main]
...
### object-type update 说明

- **`--add-property`**：可重复；每条为描述一个数据属性的 JSON 对象（需含 `name`）；同名则覆盖。
- **`--remove-property`**：按 `name` 从 `data_properties` 中移除。
- **`--tags`**：替换标签，值为 JSON 字符串数组，如 `'["足球","球员"]'`。
- **原始 JSON**：第三个参数（或仅用 `kweaver call` PUT）时，请求体需与平台一致，且 **`kweaver call -d` 会自动带 `Content-Type: application/json`**（未手动指定 `-H Content-Type` 时）。
